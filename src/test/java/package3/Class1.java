...
public class Class1 extends CommonClass {
	...
	@Test(groups = { "Group1_in_package3","Y" },description = "Rupesh",priority=1)
	public void method1_in_Class1_in_package3(Method method) throws IOException {
		System.out.println("\t \t \t \t method1_in_Class1_in_package3()");
	...
	public void method2_in_Class1_in_package3(Method method) throws IOException {
		System.out.println("\t \t \t \t method2_in_Class1_in_package3()");
		childThread.get().assignAuthor(method.getAnnotation(Test.class).description());
		childThread.get().assignCategory(method.getAnnotation(Test.class).groups());
	...
	@Test(groups = { "Group3_in_package3", "Y" }, description = "Kiran", priority = 3)
	public void method3_in_Class1_in_package3(Method method) throws IOException {
		System.out.println("\t \t \t \t method3_in_Class1_in_package3()");
		childThread.get().assignAuthor(method.getAnnotation(Test.class).description());
		childThread.get().assignCategory(method.getAnnotation(Test.class).groups());
		childThread.get().pass("Step 1 in " + method.getName() + " executed successfully!", MediaEntityBuilder
				.createScreenCaptureFromPath(someScreenshotPath, "Sample Screenshot attached.").build());
